	...
	Budget.prototype = {
		...
		addNewIncome: function(e) {
			if (e.target && e.target.id === this.incID) {
				...
				if (((!isNaN(value) && value.toString().indexOf('.') != -1) || value % 1 === 0) && value > 0) {
					this.addIncomeToTable(this.addData("income", incomeName.value, incomeAmt.value));
				}
				else {
					// invalid input
		...
		addNewExpense: function(e) {
			if (e.target && e.target.id === this.expID) {
				...
				if (((!isNaN(value) && value.toString().indexOf('.') != -1) || value % 1 === 0) && value > 0) {
					this.addExpenseToTable(this.addData("expense", expenseName.value, expenseAmt.value));
				}
				else {
					// invalid input
					if (this.data.warning === "on") {
		...
		removeIncome: function(e) {
			...
			if (e.target) {
				...
				for (i = 0; i < this.data.items["income"].length; i++) {
					if (row && e.target.id === (this.incID+this.data.items["income"][i].id).toString()) {
		...
		removeExpense: function(e) {
			...
			if (e.target) {
				...
				for (i = 0; i < this.data.items["expense"].length; i++) {
					if (row && e.target.id === (this.expID+this.data.items["expense"][i].id).toString()) {
		...
		generateReport: function(e) {
			if (e.target && e.target.id === (this.incID + this.expID + "gr")) {
				...
				const text = "<b>" + this.name + "</b>" + "<br>"
				+ "Available Budget: " + this.data.balance.toString() + "<br>"
				+ "Total Income: " + totalIncome.toString() + "<br>"
				+ "Total Expense: " + totalExpenses.toString() 
				+ " (" + ratio.toString() + "% of income)" + "<br>";
...
/* DOM Functions */
		...
		makeIncomeTable: function() {
			...
			addIncomeRow.className = "addForm";
		...
		makeExpenseTable: function() {
			...
			addExpenseRow.className = "addForm";
		...
		updateBalance: function() {
			...
			if ((totalIncome - this.data.balance) > totalIncome) {
				document.getElementById(this.incID + this.expID).style.color= "red";
				if (this.data.warning === "on") {
			...
			else if (((totalIncome - this.data.balance) / totalIncome) < this.data.ratio) {
				document.getElementById(this.incID + this.expID).style.color= "green";
			}

			// expense/income greater than ratio but less than 1
			else if (((totalIncome - this.data.balance) / totalIncome) >= this.data.ratio) {
				document.getElementById(this.incID + this.expID).style.color= "#FFB61E";
				if (this.data.warning === "on") {
					this.inputWarning("Expense to income ratio is greater than the specified limit of "
		...
		addIncomeToTable: function(incObj) {
			...
			newIncomeRow.innerHTML = `<td><button class="del" id=${this.incID+(incObj.id).toString()}>x</button>
		...
		addExpenseToTable: function(expObj) {
			...
			newExpenseRow.innerHTML = `<td><button class="del" id=${this.expID+(expObj.id).toString()}>x</button>
		...
		inputWarning: function(text) {
			...
			inputWarning.innerHTML = `<button id=${this.incID}show class="hideWarning">Don't show again</button>
		...
		hideWarning: function(e) {
			if (e.target && e.target.id === ((this.incID) + "show")) {
				...
				document.getElementById((this.incID) + "show").parentElement.style.display = "none";
			}
		}
	}
